...
func TestReleaseOnce(t *testing.T) {
	r := New(&struct{}{}, noopCloser)

	err := r.Release()
	require.NoError(t, err)

	err = r.Release()
	require.Equalf(t, ErrReleased, err, "expected ErrReleased, actual: %s", err)
...
func TestValue(t *testing.T) {
	...
	func() {
		defer func() {
			require.Equalf(t, ErrReleased, recover(), "expected panic with ErrReleased, actual: %v", recover())
		}()
...
func TestClosedOnce(t *testing.T) {
	...
	root := New(&struct{}{}, closer)

	references := make([]*Reference[*struct{}], 25)
