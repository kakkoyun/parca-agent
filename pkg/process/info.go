...
import (
	...
	"github.com/parca-dev/parca-agent/pkg/container"
)

// TODO(kakkoyun): Refactor to use procfs or eliminate this file.
// - Check out metadata/process.go
...
type Info struct {
	procInfoMap map[int]Context
	mtx         sync.RWMutex // protecting both update and delete entries
}

func (p *Info) UpdateElement(hostTid int, ctx Context) {
	p.mtx.Lock()
	defer p.mtx.Unlock()
	p.procInfoMap[hostTid] = ctx
}

func (p *Info) DeleteElement(hostTid int) {
	p.mtx.Lock()
	defer p.mtx.Unlock()
	delete(p.procInfoMap, hostTid)
}

func (p *Info) GetElement(hostTid int) (Context, error) {
	p.mtx.RLock()
	defer p.mtx.RUnlock()
	processCtx, ok := p.procInfoMap[hostTid]
	if !ok {
		return Context{}, fmt.Errorf("no such a key: %v", hostTid)
	}
	return processCtx, nil
}

func ParseProcessContext(ctx []byte, containers *container.Containers) (Context, error) {
	var procCtx = Context{}
	const ctxSize = 52
	if len(ctx) < ctxSize {
	...
	procCtx.HostPid = binary.LittleEndian.Uint32(ctx[28:32])
	procCtx.HostTid = binary.LittleEndian.Uint32(ctx[32:36])
...
func parseProcStatus(status []byte, taskStatusPath string) (Context, error) {
...
func NewProcessInfo() (*Info, error) {
	p := Info{make(map[int]Context), sync.RWMutex{}}
